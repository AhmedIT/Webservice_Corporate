...
add_log()
{
  ...
  if  [ ! -f $log_file ]; then
	...
	message=$(echo "$2" | tr "\n" " ")
  echo "$(date '+%Y-%m-%d %H:%M:%S');storelocator;prod;[$bashname] - $message;;CRP;CRPWS;storelocator_bash;en_US;;" >> $log_file
...
sphinx_rotate()
{  
	cmd_rotate=$(/usr/bin/indexer prod --rotate --config $target_conf_file)
	case "$cmd_rotate" in
		*succesfully*) add_log "DEBUG" "Rotating OK prod index rotated : $cmd_rotate";;
...
show_debug=1 
  ...
  lookup_conf_file="/data/www/ws.chanel.com/www/sources/storelocator/conf/prod/sphinx.conf.php"
  target_conf_file="/etc/sphinx/sphinx.conf"
  backup_conf_file="/data/www/ws.chanel.com/www/sources/storelocator/conf/prod/sphinx.conf.$(date '+%Y%m%d%H%M%S').php"
  lookup_rotate_file="/data/www/ws.chanel.com/www/sources/storelocator/sphinx_rotate"
